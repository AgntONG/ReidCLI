...
(Researcherâ†’Plannerâ†’Critic planning pipeline), Nyx (redteam persona mode),
...
```powershell
npm install -g reid-cli
```

This installs the `reid-cli` package and creates two commands on your `PATH`:
- `reid` â€” short alias
- `reid-cli` â€” full command name

Skip to [step 2](#2-verify-the-install) to check it worked.
...
- **Collapsible reasoning + tool calls (Ctrl+O)** â€” chain-of-thought shows as
  a grayed-out `âœ» Thought for Ns` line and each tool call as a one-line
  `â— tool(args) ok/error` summary, collapsed by default. `Ctrl+O` toggles
  every collapsed block open at once to see the full detail.
- **`/` completion menu** â€” type `/` for a live menu of every slash command
  with its description (Tab/â†“/â†‘ to navigate, Enter to accept); no need to run
...
- **Escape to stop a response** â€” while a reply is generating, `Esc` cancels
  just that turn (the spinner switches to `â— stoppingâ€¦`) and returns whatever
...
- **DeepReid trigger** â€” type `deepread`/`deep reid` (a few spellings
  ...
  runs through the real Researcherâ†’Plannerâ†’Critic pipeline instead of a
...
- **Keyboard shortcuts in the input box:**
  - `â†‘` / `â†“` â€” input history
  - `â†` / `â†’` â€” cycle reasoning effort (`low â†’ medium â†’ high â†’ xhigh`) when
...
| Command | Purpose |
|---|---|
| `reidcli` | Launch the interactive TUI (default â€” no subcommand needed) |
| `reidcli interactive "<prompt>"` | Launch interactive mode and immediately submit `<prompt>` as the first turn â€” session stays open afterward |
| `reidcli --file <path>` / `-f` | Same idea, but read the initial prompt from a text file â€” works with `interactive`, `exec`, and the bare/no-subcommand form |
...
| `reidcli deepreid "<task>"` | Plan + review `<task>` via the Researcher/Planner/Critic pipeline (headless, like `exec`) â€” no code changes, saves a Markdown plan |
...
A real Researcherâ†’Plannerâ†’Critic subagent pipeline for planning and
...
Config is merged in this precedence order (low â†’ high):

1. **Built-in defaults** â€” a stub provider, balanced mode
2. **Global config** â€” `~/.reidcli/config.json`
3. **Project config** â€” `./.reidcli/config.json`
...
2. A project-local `settings.json`, found by walking upward from the current
   directory the way `git` finds `.git` — so launching `reid-cli` from any
...
| `REIDCLI_SETTINGS` | Explicit path override for the `settings.json` lookup above |
...
- `reidcli-build-plan.md` â€” full product definition and phase plan
- `agent-first-cli-spec.md` â€” generic agent-first CLI specification
- `docs/reidcli-architecture-audit.md` â€” file-aware critique of this scaffold
- `docs/reidcli-phase-5-plan.md` â€” correctness fixes + interaction upgrade
...
- Task tracking with status state machine (pending â†’ active â†’ completed/failed/skipped)
...
- **DeepReid** (`src/reidcli/deepreid/`): a real Researcherâ†’Plannerâ†’Critic
